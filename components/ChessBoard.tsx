...
function squareName(r: number, c: number) { return files[c] + (8 - r) }
...
export default function ChessBoard({ boardState, onMove }: Props) {
    const board = useMemo(() => decodeBoard(boardState), [boardState])
    ...
    // Optional: Add a tiny bit of spring for a "premium" feel
    ...
    useEffect(() => {
        ...
        // Global move listener that works EVEN DURING NATIVE DRAG
    ...
    return (
        <div className="relative select-none">
            <div
                className="grid grid-cols-8 w-[480px] max-w-full aspect-square bg-neutral-900 p-1 shadow-2xl"
            >
                {board.map((row, r) =>
                    row.map((piece, c) => {
